...
# Python/FastAPI Development Workflow (AI-Commerce v2.0)
...
> **⚠️ AI-Commerce Project Rule**: This project uses a strict layered architecture. See `architecture-patterns` skill for the 7 Core Pillars. All DB access goes through `repositories/`, entities are pure business logic, and controllers are thin.
...
- `architecture-patterns` - Repository Pattern enforcement

> **❌ Do NOT use Prisma** — This project uses SQLAlchemy 2.0 exclusively.

#### Actions
1. Design database schema in `common/models/`
2. Set up SQLAlchemy models with `Mapped` type annotations
3. Create database connection in `common/`
4. Configure migrations with Alembic
5. Create repositories in `backend/repositories/`

#### Copy-Paste Prompts
```
Use @pydantic to create Pydantic schemas for the API in backend/schemas/
```

### Phase 3: API Routes (Thin Controllers + Explicit Routers)

#### Skills to Invoke
- `architecture-patterns` - Thin Controller pattern
- `code-review-excellence` - Validate dependency boundaries

#### Actions
1. Create API routers in `backend/routers/` (HTTP metadata only)
2. Create controllers in `backend/controllers/` (NO decorators, pure Python)
3. Implement repositories in `backend/repositories/` (ONLY DB access)
4. Add Pydantic request/response schemas in `backend/schemas/`
5. Wire routers → controllers → repositories

#### Copy-Paste Prompts
```
Use @architecture-patterns to create thin controllers with Repository Pattern
```
