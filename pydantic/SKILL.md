...
> **⚠️ AI-Commerce Project Rule**: In this project, all DB access goes through `backend/repositories/`. Pydantic schemas are used for **API request/response validation** and for converting ORM models returned by repositories into typed responses. Never write standalone functions that take `Session` directly — that's the Repository's job.

### ORM Models with Pydantic (Repository Pattern)

```python
# common/models/user.py — SQLAlchemy model (DB schema only)
...
# backend/schemas/user.py — Pydantic schemas (API validation)
...
# backend/repositories/user_repository.py — ONLY layer touching DB
...
# backend/controllers/user.py — thin, testable
from backend.repositories.user_repository import UserRepository
from backend.schemas.user import UserResponse

def get_user_controller(user_id: int, session=Depends(get_session)):
    repo = UserRepository(session)
    user = repo.get_by_id(user_id)
    return UserResponse.model_validate(user)  # ORM → Pydantic
```

### Pydantic Schema Variants (Create / Update / Response)
...
# ✅ CRUD operations live in repositories, NOT in standalone functions
# See backend/repositories/ for the actual DB operations
```
